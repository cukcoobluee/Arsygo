...
<section class="max-w-7xl mx-auto px-6 md:px-10  mb-10">
  <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
    ...
    <div class="bg-green-50 border border-green-200 shadow-sm rounded-xl p-6">
      ...
      <ul class="harga-termasuk-list space-y-2 text-gray-700 text-sm md:text-base">
...
<script>
  ...
  const fasilitasList = document.querySelector('.harga-termasuk-list'); // ubah class di HTML ul jadi ini
  ...
  function getHarga(peserta) {
    if (peserta >= 20) return 325000;
    if (peserta >= 7) return 249000;
    return 500000;
  }

  // 🔹 Data fasilitas sesuai jumlah orang
  const paketFasilitas = {
    kecil: [ // min 7 orang
      "Mobil Landrover",
      "Driver profesional berlisensi",
      "Bahan bakar (bbm)",
      "Tiket masuk kawasan wisata hutan",
      "P3K / Safety Equipment",
      "Meeting Point / Penjemputan area Lembang",
      "Asuransi",
      "Mineral water"
    ],
    sedang: [ // min 20 orang
      ...
      "Driver profesional berlisensi",
      "Bahan bakar (bbm)",
      "Meals: 1x Makan siang, 1x Coffee Break",
      "Tiket masuk kawasan wisata hutan",
      "P3K / Safety Equipment",
      "Meeting Point / Penjemputan area Lembang",
      "Asuransi",
    ...
    besar: [ // min 20 orang dengan paintball
      "Mobil Landrover",
      "Driver profesional berlisensi",
      "Bahan bakar (bbm)",
      "Meals: 1x Makan siang, 1x Coffee Break",
      "Paintball war 40 peluru",
  ...
  // 🔹 Fungsi ganti fasilitas
  function updateFasilitas(peserta) {
    let fasilitas;
    if (peserta >= 20 && peserta < 30) fasilitas = paketFasilitas.sedang;
    else if (peserta >= 30) fasilitas = paketFasilitas.besar;
    else fasilitas = paketFasilitas.kecil;

    fasilitasList.innerHTML = fasilitas
      .map(item => `<li class="flex items-start"><span class='text-green-500 mr-2'>•</span>${item}</li>`)
      .join('');
  }

  // 🔹 Update harga + fasilitas saat slider digerakkan
  slider.addEventListener('input', function() {
    ...
    updateFasilitas(peserta);
  });

  // 🔹 Jalankan default saat pertama kali load
  updateFasilitas(parseInt(slider.value));
</script>
</body>